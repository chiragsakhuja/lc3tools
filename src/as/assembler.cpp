...
bool Assembler::processInstruction(  bool print_enable, AssemblerPrinter const & printer, InstructionEncoder & encoder
                                   ...
                                   , std::map<std::string, int> const & symbol_table, uint32_t & encoded_instruction) const
{
    ...
    if(! foundMatch) {
        ...
        if(potentialMatch == nullptr) {
            ...
            if(print_enable) {
                printer.printfAssemblyMessage(AssemblerPrinter::ERROR, filename, inst, file_buffer[inst->row_num], "incorrect number of operands for instruction \'%s\'", inst->str.c_str());
            }
        } else {
            ...
            if(print_enable) {
                for(auto it = potentialMatch->oper_types.begin(); it != potentialMatch->oper_types.end(); it++) {
                    if(! (*it)->compareTypes(cur->type)) {
                        printer.printfAssemblyMessage(AssemblerPrinter::ERROR, filename, cur, file_buffer[inst->row_num], "incorrect operand");
                    }
        ...
        success &= encoder.encodeInstruction(print_enable, printer, potentialMatch, inst, encoded_instruction);
    }
...
bool Assembler::getOrig(bool print_enable, AssemblerPrinter const & printer, const std::string& filename, const Token *orig, int& newOrig)
{
    if(orig->checkPseudoType("orig")) {     // sanity check...
        if(print_enable && orig->num_operands != 1) {
            printer.printfAssemblyMessage(AssemblerPrinter::ERROR, filename, orig, file_buffer[orig->row_num], "incorrect number of operands");
            return false;
        } else {
            if(print_enable && orig->opers->type != NUM) {
                printer.printfAssemblyMessage(AssemblerPrinter::ERROR, filename, orig->opers, file_buffer[orig->row_num], "illegal operand");
                return false;
            } else {
...
bool Assembler::processPseudo(bool print_enable, AssemblerPrinter const & printer, const std::string& filename, const Token *pseudo)
{
    if(pseudo->checkPseudoType("orig")) {
        getOrig(print_enable, printer, filename, pseudo, sectionStart);
...
bool Assembler::processTokens(bool print_enable, AssemblerPrinter const & printer, InstructionEncoder & encoder, std::string const & filename, Token * program, std::map<std::string, int> & symbol_table, Token *& program_start)
{
    ...
    while(cur_state != nullptr && ! found_valid_orig) {
        ...
        while(cur_state != nullptr && ! cur_state->checkPseudoType("orig")) {
            ...
            if(print_enable) {
                printer.xprintfAssemblyMessage(AssemblerPrinter::WARNING, filename, 0, file_buffer[cur_state->row_num].length(), cur_state, file_buffer[cur_state->row_num], "ignoring statement before valid .orig");
            }
        ...
        if(getOrig(print_enable, printer, filename, cur_state, cur_orig)) {
    ...
    while(cur_state != nullptr) {
        ...
        if(cur_state->type == LABEL) {
            ...
            if(print_enable) {
                printer.printfMessage(AssemblerPrinter::DEBUG, "setting label \'%s\' to 0x%X", label.c_str(), cur_orig + pc_offset);
            }
        }
...
bool Assembler::assembleProgram(bool print_enable, Printer const & printer, std::string const & filename, Token * program, std::map<std::string, int> & symbol_table)
{
    std::ifstream file(filename);
    AssemblerPrinter assembler_printer;
    InstructionEncoder encoder(print_enable, printer);
    bool success = true;
    ...
    if(print_enable) {
        assembler_printer.printfMessage(AssemblerPrinter::INFO, "beginning first pass ...");
    }

    Token * cur_state = nullptr;
    if(! processTokens(print_enable, assembler_printer, encoder, filename, program, symbol_table, cur_state)) {
        return false;
    }

    if(print_enable) {
        assembler_printer.printfMessage(AssemblerPrinter::INFO, "first pass completed successefully, beginning second pass ...");
    }

    while(cur_state != nullptr) {
        if(cur_state->checkPseudoType("orig")) {
            success &= processPseudo(print_enable, assembler_printer, filename, cur_state);
        }

        if(cur_state->type == INST) {
            uint32_t encoded_instruction;

            success &= processInstruction(print_enable, assembler_printer, encoder, filename, cur_state, symbol_table, encoded_instruction);
        }
        cur_state = cur_state->next;
    }

    if(success && print_enable) {
        assembler_printer.printfMessage(AssemblerPrinter::INFO, "second pass completed successfully");
    }
